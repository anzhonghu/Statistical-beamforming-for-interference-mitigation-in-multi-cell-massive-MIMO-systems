...
SNR_n = 5;
SNR_store = [-20; -15; -10; -5; 0;];
SINR_store = zeros(N_total, 2); % 2 approaches
SINR_95_store = zeros(SNR_n, 2);
...
for snr_n = 1 : SNR_n
    snr = SNR_store(snr_n, 1);%dB, received SNR
    for ii = 1 : Ite_num
    ...
    SINR_95_store(snr_n, :) = SINR_store(N_total*0.05, :);
end
...
set(h,'PaperType','A4');
xx = axes('FontSize',16);
plot(SNR_store, SINR_95_store(:, 1), '--^','LineWidth',2,'MarkerSize',14)
hold on
plot(SNR_store, SINR_95_store(:, 2), 'k-s','LineWidth',2,'MarkerSize',14)
grid on
le = legend('the approach in [15]','the proposed approach', 'Location', 'northwest');
set(le,'Fontsize',16,'Fontname','Times')
set(gca,'XTick',SNR_store)
xlabel('The received SNR (dB)','Fontsize',20,'Fontname','Times')
ylabel('P \geq 0.95 SINR (dB)','Fontsize',20,'Fontname','Times')
%print(h,'-dpdf','sinr_snr')
%%%%%%%%%%%%%%%%%%%
