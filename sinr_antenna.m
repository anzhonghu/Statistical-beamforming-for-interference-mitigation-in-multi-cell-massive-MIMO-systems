...
antenna_n = 5;
antenna_store = [49; 100; 169; 225; 289];
SINR_store = zeros(N_total, 2); % 2 approaches
SINR_95_store  = zeros(antenna_n, 2); %5 numbers of antennas, 2 approaches
snr = 10;
...
for a_n = 1 : antenna_n
    M = antenna_store(a_n, 1);%dB, received SNR
    Mx = sqrt(M);
    My = Mx;
    ...
    SINR_store = sort(SINR_store, 1);
    SINR_95_store(a_n, :) = SINR_store(N_total*0.05, :);
end
SINR_95_store = 10 * log10(SINR_95_store);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
h = figure;
set(h,'PaperType','A4');
xx = axes('FontSize',16);
plot(antenna_store, SINR_95_store(:, 1), '--^','LineWidth',2,'MarkerSize',14)
hold on
plot(antenna_store, SINR_95_store(:, 2), 'k-s','LineWidth',2,'MarkerSize',14)
grid on
le = legend('the approach in [15]','the proposed approach', 'Location', 'northwest');
set(le,'Fontsize',16,'Fontname','Times')
set(gca,'XTick',antenna_store)
xlabel('The number of URA antennas','Fontsize',20,'Fontname','Times')
ylabel('P \geq 0.95 SINR (dB)','Fontsize',20,'Fontname','Times')
%print(h,'-dpdf','sinr_antenna')
%%%%%%%%%%%%%%%%%%%
