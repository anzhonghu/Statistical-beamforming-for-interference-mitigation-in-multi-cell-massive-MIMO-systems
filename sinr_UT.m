...
for ut_n = 1 : UT_n
    ...
    SINR_95_store(ut_n, :) = SINR_store(N_total*0.05, :);
end
...
set(h,'PaperType','A4');
xx = axes('FontSize',16);
plot(UT_store, SINR_95_store(:, 1), '--^','LineWidth',2,'MarkerSize',14)
hold on
plot(UT_store, SINR_95_store(:, 2), 'k-s','LineWidth',2,'MarkerSize',14)
grid on
le = legend('the approach in [15]','the proposed approach');
set(le,'Fontsize',16,'Fontname','Times')
set(gca,'XTick',UT_store)
xlabel('The number of UTs in each sector','Fontsize',20,'Fontname','Times')
ylabel('P \geq 0.95 SINR (dB)','Fontsize',20,'Fontname','Times')
%print(h,'-dpdf','sinr_UT')
%%%%%%%%%%%%%%%%%%%
