...
Mx = sqrt(M);
My = Mx;
theta_d = 5;%angular deviation
phi_d = 2;%angular deviation
a = zeros(M, 1);%steering vector
N = zeros(M, Nt);%received noise at the BS
L = 3;%cell number
SINR_store = zeros(Ite_num * Nt * L, 8);
...
for ii = 1 : Ite_num
    ...
    Sk = sign(randn(L, K*Nt)) * amp_k;%BPSK modulation
    for j = 1 : L
    ...
    for j = 1 : L
        ...
        b_te = S_d' * Q_Null;
        b_te = b_te .* conj(b_te);
        [b_te_max, b_te_ind] = max(b_te);
        w = Q_Null(:, b_te_ind);
        w = w / norm(w);
        Y1_b = w' * Y1;
        S1_b = w' * YS1(:, 1:Nt);
        SINR_store((ii-1)*Nt*L + (j-1)*Nt + 1: (ii-1)*Nt*L + j*Nt, 3) = (abs(S1_b).^2 ./ (abs(Y1_b - S1_b).^2))';
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        Q_s = zeros(M, M);
        a_temp = zeros(M, 1);
        ...
        Y1_b = w' * Y1;
        S1_b = w' * YS1(:, 1:Nt);
        SINR_store((ii-1)*Nt*L + (j-1)*Nt + 1: (ii-1)*Nt*L + j*Nt, 4) = (abs(S1_b).^2 ./ (abs(Y1_b - S1_b).^2))';
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        Q_s3 = zeros(M, 1);
        for l = 1 : L
            if j == l
        ...
        Y1_b = w' * Y1;
        S1_b = w' * YS1(:, 1:Nt);
        SINR_store((ii-1)*Nt*L + (j-1)*Nt + 1: (ii-1)*Nt*L + j*Nt, 5) = (abs(S1_b).^2 ./ (abs(Y1_b - S1_b).^2))';
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         w = w + Q_sa / norm(Q_sa);
%         Y1_b = w' * Y1;
%         S1_b = w' * YS1(:, 1:Nt);
%         SINR_store((ii-1)*Nt*L + (j-1)*Nt + 1: (ii-1)*Nt*L + j*Nt, 6) = (abs(S1_b).^2 ./ (abs(Y1_b - S1_b).^2))';
%         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         w = Q_sa / norm(Q_sa);
%         Y1_b = w' * Y1;
%         S1_b = w' * YS1(:, 1:Nt);
%         SINR_store((ii-1)*Nt*L + (j-1)*Nt + 1: (ii-1)*Nt*L + j*Nt, 7) = (abs(S1_b).^2 ./ (abs(Y1_b - S1_b).^2))';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            [V_g, D_g] = eig(Q_I, Q_s);
            D_g = diag(D_g);
            [min_g, index_g] = min(D_g);
            w = V_g(:, index_g);
            w = w / norm(w);
            Y1_b = w' * Y1;
            S1_b = w' * YS1(:, 1:Nt);
            SINR_store((ii-1)*Nt*L + (j-1)*Nt + 1: (ii-1)*Nt*L + j*Nt, 8) =  (abs(S1_b).^2 ./ (abs(Y1_b - S1_b).^2))';
    end
    display(ii);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
semilogx(SINR_store(:,1),[1:Ite_num * Nt * L]/(Ite_num * Nt * L))
hold on
%semilogx(SINR_store(:,4),[1:Ite_num * Nt * L]/(Ite_num * Nt * L), '--')
semilogx(SINR_store(:,5),[1:Ite_num * Nt * L]/(Ite_num * Nt * L), 'r-')
% semilogx(SINR_store(:,6),[1:Ite_num * Nt * L]/(Ite_num * Nt * L), 'g--')
% semilogx(SINR_store(:,7),[1:Ite_num * Nt * L]/(Ite_num * Nt * L), 'k--')
semilogx(SINR_store(:,8),[1:Ite_num * Nt * L]/(Ite_num * Nt * L), 'ko-')
legend('[10]的方法','所提方法','另一现有方法')
xlabel('信干比')
ylabel('累积分布')
%%%%%%%%%%%%%%%%%%%
